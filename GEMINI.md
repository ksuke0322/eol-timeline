...
3. 全タスク消化後、以下のコマンドを全て実行しすべて正常終了することを確認してください。（以降、このコマンドのことを動作確認コマンドと呼びます）異常終了した場合は問題箇所を特定・修正し、1つ目のコマンドに戻って実行し直してください。
   1. pnpm lint -> error が出ていないことを確認してください。（warning はスキップ）
   2. pnpm tsc -> 成功することを確認してください。
   3. pnpm test -> 全てのテストが成功することを確認してください。
   4. pnpm build -> 成功することを確認してください。
   5. pnpm test-storybook -> 全てのテストが成功することを確認してください。（a11y の問題はスキップ）
   6. pnpm test:e2e -> 全てのテストが成功することを確認してください。
...
- 対象
  ...
  - コンポーネントの a11y の violation 確認（storybook同様UIパターンを網羅）
- 特徴：
